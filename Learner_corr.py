...
class face_learner(object):
        ...
        if not inference:
            ...
            if conf.use_mobilfacenet:
                self.optimizer = optim.SGD([
                                               ...
                                               {'params': [paras_wo_bn[-1]] + [self.head[head_num].kernel],
                                                'weight_decay': 4e-4}
                ...
                self.optimizer = optim.SGD([
                                               ...
                                               {'params': paras_wo_bn + [self.head[head_num].kernel],
                                                'weight_decay': 5e-4}
                                               for head_num in range(conf.n_models)
                                           ], lr=conf.lr, momentum=conf.momentum)
    ...
    def save_state(self, conf, accuracy, to_save_folder=False, extra=None, model_only=False):
        ...
        if not model_only:
            for mod_num in range(conf.n_models):
                torch.save(
                    self.head[mod_num].state_dict(), save_path /
    ...
    def load_state(self, conf, fixed_str, from_save_folder=False, model_only=False):
        ...
        if not model_only:
            for mod_num in range(conf.n_models):
                self.head[mod_num].load_state_dict(torch.load(save_path / 'head_{}_{}'.format(mod_num, fixed_str)))
    ...
    def find_lr(self, conf, init_value=1e-8, final_value=10., beta=0.98, bloding_scale=3., num=None):
        ...
        for i, (imgs, labels) in tqdm(enumerate(self.loader), total=num):
            ...
            for model, head in zip(self.models, self.heads):
                embeddings.append(model(imgs))
                thetas = head(embeddings, labels)
                joint_losses.append(conf.ce_loss(thetas, labels))

            if conf.pearson:
                outputs = torch.stack(embeddings)
                ...
                mean_output = torch.mean(torch.stack(embeddings), 0)
