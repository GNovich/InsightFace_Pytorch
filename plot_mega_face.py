...
from megaface_interpolation import linear_interp, linear_interp_logx, nearest_neighbor_interp
...
#    print 'cmc_files: ', cmc_files
    ...
    for i, filename in enumerate(cmc_files):
        n_distractors = int(filename.split('_')[-2])
        with open(os.path.join(folder, filename), 'r') as f:
            cmc_dict[n_distractors] = json.load(f)

    rocs = []

    for i in cmc_dict.keys():
    ...
    rank_1 = [cmc_dict[n]['cmc'][1][0] for n in cmc_dict]

    rank_10 = []
    for i in cmcs:
        target_recall = interp_target_rank_recall(i, 10)
    ...
    n_distractors = list(cmc_dict.keys())
    return {
        'rocs': rocs,
        'cmcs': cmcs,
        'Rank_1': rank_1,
...
def calc_target_tpr_and_rank(rocs, rank_1, rank_10, save_dir,
                             ...
                             fp_tpr_sum=None, fp_rank_sum=None, n_distractors=[1000000]):
...
def calc_open_set(rocs, rank_1, rank_10, save_dir,
                             method_label=None, target_fpr=1e-6,
                             fp_tpr_sum=None, fp_rank_sum=None, n_distractors=[1000000]):
...
def plot_megaface_result(your_method_dirs, your_method_labels,
    ...
    for j in range(n_results):
        ...
        n_distractors.append(your_result['n_distractors'])
        ...
        calc_target_tpr_and_rank(rocs, rank_1, rank_10,
                                 ...
                                 n_distractors=n_distractors[j])

        calc_open_set(rocs, rank_1, rank_10,
                                 save_dir, your_method_labels[j],
                                 target_fpr,
                                 fp_tpr_sum=fp_tpr_sum,
                                 fp_rank_sum=fp_rank_sum,
        ...
        labels = [str(it) for it in n_distractors[j]]
        ...
        color_idx = 0
        for i in range(len(n_distractors[j])):
...
#            print osp.realpath(other_methods_dir)
    ...
    for i, pn in enumerate(plot_names):
        ...
        for j in range(n_results):
            ...
            ax.semilogx(your_methods_data[j]['rocs'][i][0],
                        your_methods_data[j]['rocs'][i][1],
        ...
        if other_methods_list:
            ...
            for name in other_methods_list:
                ...
                ax.semilogx(other_methods_data[name]['rocs'][i][0],
                            other_methods_data[name]['rocs'][i][1],
                            label=name,
                            c=_color)
                color_idx += 1
    ...
    #    plt.legend()
        ...
        for j in range(n_results):
            ...
            ax.semilogx(your_methods_data[j]['cmcs'][i][0],
                        your_methods_data[j]['cmcs'][i][1],
                        label=your_method_labels[j],
                        c=_color)
            color_idx += 1
        ...
        if other_methods_list:
            ...
            for name in other_methods_list:
                ax.semilogx(other_methods_data[name]['cmcs'][i][0],
                            other_methods_data[name]['cmcs'][i][1],
                            label=name,
                            c=np.random.rand(3))
    ...
    for j in range(n_results):
        ...
        ax.semilogx(n_distractors[j], your_methods_data[j]['Rank_1'],
                    label=your_method_labels[j],
                    c=_color)
        color_idx += 1
...
if __name__ == '__main__':
    your_method_labels = os.listdir('results')
    your_method_dirs = [os.path.join('results', x) for x in your_method_labels]
    ...
    for probe_name in probesets:
        save_dir = './megaface_%s_insightface' % probe_name
